...
export function notaTematica(ente: Ente, tagSlug: string): number | null {
  const nota = notaPorUnidadeTag.get(`${ente.codigo}|${tagSlug}`)
...
export function rankingTematico(
  ...
  return [...entes]
    .map(e => {
      ...
      return valor == null ? null : { slug: e.slug, nome: e.nome, valor }
    })
    .filter(
      (x): x is { slug: string; nome: string; valor: number } => x !== null
    )
